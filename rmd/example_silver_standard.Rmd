...
```{r install_prroc}
devtools::install_github('liangyy/prroc')
library(prroc)
```

# Load silver standard

Download silver standard [here](https://github.com/hakyimlab/gold-standard/blob/master/data/silver_phecode_to_mim.rds) 

```{r load_silver}
silver_standard = readRDS('~/Downloads/silver_phecode_to_mim.rds')
# show the silver standard object
silver_standard$script_info
silver_standard$table %>% head %>% pander
```
...
```{r silver1}
o = silver_standard_proto(score_table, map_table, silver_standard, trait_code = 'phecode')
...
```{r silver2}
o = silver_standard_proto(score_table, map_table2, silver_standard, trait_code = 'HPO')
